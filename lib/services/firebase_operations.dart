...
Future<void> saveUserStateToFirestore(AuthState authState, UserState userState) {
	return _users
		.doc( authState.uid )
		.set({
			'username': userState.username,
			'totalQuestionsAnswered': userState.totalQuestionsAnswered,
			'totalQuestionsAnsweredCorrectly': userState.totalQuestionsAnsweredCorrectly,
		...
		.catchError((error) { Fluttertoast.showToast(msg: "Data saving failed. Error: $error"); });
}
...
Future<void> loadUserStateFromFirestore(AuthState authState, UserState userState) {
	Fluttertoast.showToast(msg: "Fetching user data... Please wait",);
	...
	return _users
		...
		.catchError((error) { Fluttertoast.showToast(msg: "User data fetch failed. Error: $error"); });
}



Future<void> saveProfilePicToStorage(AuthState authState, UserState userState) async {
...
Future<void> loadProfilePicFromStorage(AuthState authState, UserState userState) async {
	if (authState.googleStorageProfilePicPath == null) return;

	String basename = path.basename( authState.googleStorageProfilePicPath as String );
